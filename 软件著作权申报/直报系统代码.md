# 广东省卫生健康统计信息网络直报系统数据转换接口代码

## 代码总体说明

本软件的功能是将多个从电子病历系统数据库中导出的csv数据整合后形成可以供广东省卫生健康统计信息网络直报系统导入的xlsx文件。代码文件结构如下

```txt
csvFile/
    ht_tbs_record.csv
    ndns.csv
    ht.bl_tbs_advtemp.csv
xmlFiles/
    para.xlsx
feiyong_parse.py
hisBA1parser.py
...
其中，`csvFile`目录存储从电子病历系统导出的多个csv文件：
1. `ht_tbs_record.csv`是存储首页的表的导出文件
2. `ndns.csv`是患者收费汇总数据表的导出文件
3. `ht.bl_tbs_advtemp.csv`是患者临时医嘱的数据导出文件
上述数据属于敏感医疗数据，在本著作权申请中不予提供

`xmlFiles`目录下的`para.xlsx`文件负责存储本软件的各项参数

代码文件：
1. `hisBA1parser.py` 主要的程序文件
